...
// Sends:
//   1. Admin notification → officialscape3d@gmail.com + officialscape3d@gmail.com
//   2. Customer confirmation → client's email
// =============================================
...
function validateStep(step) {
  ...
  if (step === 2) {
    ...
    if (!parseInt(qty.value) || parseInt(qty.value) < 1) {
      qty.classList.add('error'); if (qtyErr) qtyErr.textContent = 'Quantity must be at least 1.'; valid = false;
    } else { qty.classList.remove('error'); if (qtyErr) qtyErr.textContent = ''; }

    const desc = document.getElementById('description');
    const descErr = desc.nextElementSibling;
    if (!desc.value.trim()) {
      desc.classList.add('error'); if (descErr) descErr.textContent = 'Please describe your project.'; valid = false;
  ...
  if (orderForm) {
    orderForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        console.warn('EmailJS not configured, using mailto fallback:', err);

        // FALLBACK — opens mail client with full order details
        const subject = encodeURIComponent(`New Scape 3D Order — ${orderData.service} — ${orderData.client_name}`);
...
Submitted: ${orderData.order_date}`
        );
        window.open(`mailto:officialscape3d@gmail.com,officialscape3d@gmail.com?subject=${subject}&body=${body}`, '_blank');
        showSuccess(orderData.client_email);
      }
    });
  }
